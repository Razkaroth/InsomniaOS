...
export const SearchAndWindows = () => {
  ...
  const entry = Widget.Entry({
    ...
    onChange: (entry) => { // this is when you type
      ...
      const isDir = (['/', '~'].includes(entry.text[0]));
      const isTodo = entry.text.startsWith('td') || entry.text.startsWith('2d');
      const isTodo2Days = entry.text.startsWith('2d');
      ...
      if (isTodo) {
        ...
        if (text.startsWith('2d')) {
          todos = Todo2Day.todo_json;
        } else {
          todos = Todo.todo_json;
        }


        if (text[text.length - 1] == 'c') {
          if (dev) execAsync(['notify-send', 'Checked todos']);
          todos = todos.filter(todo => todo.done);
          if (dev) execAsync(['notify-send', `${todos.length}`].concat(todos.map(todo => todo.content)));
          resultsBox.add(CustomCommandButton({ text: 'Checked todos', noop: true }));
        } else if (text[text.length - 1] == 'u') {
          todos = todos.filter(todo => !todo.done);
          resultsBox.add(CustomCommandButton({ text: 'Unchecked todos', noop: true }));
        }
        if (todos.length == 0) {
          resultsBox.add(CustomCommandButton({ text: 'No todos found', noop: true }));
        }

        todos.forEach((todo, index) => {
          resultsBox.add(TODOCommandButton({
            text: todo.content, status: todo.done, index, callback: () => {
              resultsBox.get_children().forEach((ch, i) => {
                if (i == index) ch.destroy();
              })
            }
          }));
        });
