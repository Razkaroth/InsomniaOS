...
export const CustomCommandButton = ({ text = '', noop = false }) => searchItem({
...
export const TODOCommandButton = ({ text = '', status = false, index, name='TODO', callback }) => searchItem({
  materialIconName: status? 'check_box' : 'check_box_outline_blank',
  name: `${text}`,
  actionName: `${status ? 'Uncheck' : 'Check'}`,
  content: `${name}`,
  onActivate: () => {
    if (status) {
      todo.uncheck(index);
