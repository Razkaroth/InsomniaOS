...
export default () => {
  ...
  const ResultBox = ({ colorSystemName, updateCallback, copyCallback, setCallback }) => Box({
    children: [
      Box({
        vertical: true,
        hexpand: true,
  ...
  const resultRgb = ResultBox({
    ...
    setCallback: (self) => {
      selectedColor.setColorFromRGB(self.text, self.attribute.id);
    }
  })
  const resultHsl = ResultBox({
    ...
    setCallback: (self) => {
      selectedColor.setColorFromHSL(self.text, self.attribute.id);
    }
  })
